	...
	implements Iterable
{
	 ...
	 * @param startIndex optional parameter.
	 * @param endIndex optional parameter.
	 * 
	 * @return a {@code Array} instance with all row's elements (if any) from start index (optional) to end index (optional).
	 */
	public function getRow( n:Number, startIndex : Number, endIndex : Number ) : Array
	{
		checkGridCoords( new Point(0, n) );
		
		var a:Array = new Array();
		var l:Number = getSize().x -1;
		startIndex = ( !(isNaN(startIndex)) && (startIndex>=0) && (endIndex<=l) ) ? startIndex : 0;
		endIndex = ( !(isNaN(endIndex)) && (endIndex>=startIndex) && (endIndex<=l) ) ? endIndex : l;

		for (var i:Number = startIndex; i<=endIndex; i++) a.push( getVal( new Point(i, n)) );
		return a;
	}
	 ...
	 * @param endIndex optional parameter.
	 * 
	 * @return a {@code Array} instance with all column's elements (if any) from start index (optional) to end index (optional).
	 */
	public function getColumn( n : Number, startIndex : Number, endIndex : Number ) : Array
	{
		checkGridCoords( new Point(n, 0) );
		
		var a:Array = new Array();
		var l:Number = getSize().y -1;
		startIndex = ( !(isNaN(startIndex)) && (startIndex>=0) && (endIndex<=l) ) ? startIndex : 0;
		endIndex = ( !(isNaN(endIndex)) && (endIndex>=startIndex) && (endIndex<=l) ) ? endIndex : l;
		
		for (var i:Number = startIndex; i<=endIndex; i++) a.push( getVal( new Point(n, i)) );
		return a;
	}
	
	public function getCloneRange( from : Point, to : Point ) : AbstractGrid
	{
		var p : Point = Point.minusNew( to, from );

		if ( isGridCoords( from ) && isGridCoords( to ) && p.x >= 0 && p.y >= 0 )
		{
			var a : AbstractGrid = HashCodeFactory.buildInstance( ClassUtils.getFullyQualifiedClassName(this), [p.x+1, p.y+1] );
			...
			for( var x : Number = fromX; x<=toX; x++ ) for( var y : Number = fromY; y<=toY; y++ ) a["_aContent"][x-fromX][y-fromY] = _aContent[x][y];
			return a;
			
		} else
		{
			PixlibDebug.ERROR( this + ".getRange() failed. invalid grid coordinates : " + from + ", " + to  );
		}
	}
	
	public function getArrayRange( from : Point, to : Point ) : Array
	{
		var p : Point = Point.minusNew( to, from );

		if ( isGridCoords( from ) && isGridCoords( to ) && p.x >= 0 && p.y >= 0 )
		{
			var sizeX : Number = p.x+1;			var sizeY : Number = p.y+1;
			var a : Array = new Array( sizeX );
			for ( var i : Number = 0; i < sizeX; i++ ) a[i] = new Array( sizeY );
			...
			for( var x : Number = fromX; x<=toX; x++ ) for( var y : Number = fromY; y<=toY; y++ ) a[x-fromX][y-fromY] = _aContent[x][y];
			return a;
			
		} else
		{
			PixlibDebug.ERROR( this + ".getArrayRange() failed. invalid grid coordinates : " + from + ", " + to  );
		}
	}
