	...
	extends AbstractLib
{
	...
	public function setLoopPlayback( b : Boolean, position : Number ) : Void
	{
		if ( _bLoopPlayback != b )
		{
			...
			if ( _bLoopPlayback )
			{
				addEventListener( VideoDisplay.onEndStreamEVENT, this, _onLoop, position?position:0 );
			} else
			{
	...
	private function _onLoop( e : VideoDisplayEvent, position : Number ) : Void
	{
		this.play( position );
	}
	...
	{
		var e : CuePointEvent = new CuePointEvent( VideoDisplay.onCuePointEVENT, this, data );
		fireEvent( e );
	}
